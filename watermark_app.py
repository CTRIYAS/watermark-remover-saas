...
app = FastAPI(title="Watermark Studio", version="1.0.0")
...
def run_ffmpeg(cmd):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"FFmpeg failed: {msg[:3000]}")

@app.post("/remove")
async def remove_watermark(
    file: UploadFile = File(...),
    params: Optional[str] = Form(None),  # JSON: {"x":..,"y":..,"w":..,"h":..}
):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        out_path = Path(td) / f"cleaned_{file.filename}"

        delogo = f"delogo=x={rect['x']}:y={rect['y']}:w={rect['w']}:h={rect['h']}:show=0"
        cmd = ["ffmpeg","-hide_banner","-y","-i",str(in_path),"-vf",delogo,"-c:a","copy",str(out_path)]
        run_ffmpeg(cmd)
